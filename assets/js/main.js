...
(function () {
  ...
  /* ---------- CLIENTS ORDERS (staff-only, opens live table in a new tab) ---------- */
  (function initClientsOrders() {
    ...
    function openOrders() {
      ...
      if (!val) {
        gateMsg.textContent = "Enter the staff passcode first.";
        return;
      }
      gateMsg.textContent = "";
      window.open(`orders.html?passcode=${encodeURIComponent(val)}`, "_blank");
    }

    unlockBtn.addEventListener("click", openOrders);
    passInput.addEventListener("keydown", (e) => { if (e.key === "Enter") openOrders(); });
  })();
